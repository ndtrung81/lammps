...
class UCL_Timer {
 public:
  ...
  inline double time() {
    ...
    // According to fixes by @rrberger:
    //    removed the calls clReleaseEvent() from clear() and added them here
    // Note: clEnqueueMarker() in start() and stop() allocates resource on the host
    // clReleaseEvent() decreases the event reference counters generated by clEnqueueMarker().

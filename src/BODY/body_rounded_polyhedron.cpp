...
{
  if (narg != 3) error->all(FLERR,"Invalid body rounded/polyhedron command");
                             ...
                             int *ifile, double *dfile)
{
  ...
  // .. the next MAX_EDGE_SIZE*nedges elements are edge ends
				       ...
				       int *ifile, double *dfile)
{
  ...
  else nentries = 6 + 3*nsub + MAX_EDGE_SIZE*nedges + MAX_FACE_SIZE*nfac + 1;
  ...
  if (nsub > 2) offset += (MAX_EDGE_SIZE*nedges+MAX_FACE_SIZE*nfac);
                              ...
                              int *&ivec, double **&darray)
{
  ...
  if (nvertices == 1) { // spheres
    ...
    for (int i = 0; i < nedges; i++) {
      ...
      pt1 = static_cast<int>(edge_ends[MAX_EDGE_SIZE*i]);
      pt2 = static_cast<int>(edge_ends[MAX_EDGE_SIZE*i+1]);

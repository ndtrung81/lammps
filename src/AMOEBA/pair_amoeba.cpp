...
void PairAmoeba::init_style()
{
  ...
  index_amtype = atom->find_custom("amtype",which);
  if (index_amtype < 0 || which == 1) 
    error->all(FLERR,"Pair amoeba amtype is not defined");
  index_amgroup = atom->find_custom("amgroup",which);
  if (index_amgroup < 0 || which == 1) 
    error->all(FLERR,"Pair amoeba amgroup is not defined");
  
  index_ired = atom->find_custom("ired",which);
  if (index_ired < 0 || which == 1) 
    error->all(FLERR,"Pair amoeba ired is not defined");
  index_xaxis = atom->find_custom("xaxis",which);
  if (index_xaxis < 0 || which == 1) 
    error->all(FLERR,"Pair amoeba xaxis is not defined");
  index_yaxis = atom->find_custom("yaxis",which);
  if (index_yaxis < 0 || which == 1) 
    error->all(FLERR,"Pair amoeba yaxis is not defined");
  index_zaxis = atom->find_custom("zaxis",which);
  if (index_zaxis < 0 || which == 1) 
    error->all(FLERR,"Pair amoeba zaxis is not defined");
  
  index_polaxe = atom->find_custom("polaxe",which);
  if (index_polaxe < 0 || which == 1) 
    error->all(FLERR,"Pair amoeba polaxe is not defined");
  index_pval = atom->find_custom("pval",which);
  if (index_pval < 0 || which == 0) 

...
class PPPMDipole : public PPPM {
 public:
  ...
  // (e.g. pair gcpm/long): accumulate (+=) the reciprocal-space E-field,
  // scaled by qqrd2e, at the positions of all local atoms.
  // from_charges: field generated by the point charges (q-mu cross channel);
  //   charges are static during an SCF loop, so call once per step.
  // from_dipoles: field generated by the current atom->mu (mu-mu channel);
  //   call once per SCF iteration. Includes the dipole Ewald self-image
  //   term; the caller must apply the self-field correction.

  void compute_efield_from_charges(double **);
  void compute_efield_from_dipoles(double **);
 ...
 protected:
  ...
  void musum_musq(int errorflag = 1);

  // helpers for the field-only entry points

  void make_rho_charge();
  void make_rho_mu();
  void poisson_efield_from_charges();
  void poisson_efield_from_dipoles();
  void interpolate_efield(double **);
  void field_check();
};
